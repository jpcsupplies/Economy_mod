        ...
        public override void ProcessServer()
        {
			...
			if(Ctype == "sell")
			{
				if (Amount > 0)
				{
					...
					if (selectedShip != null)
					{
						var check = ShipManager.CheckSellOrder(selectedShip.EntityId.ToString());
						if(check == 0)
						{
			...
			else if(Ctype == "cancel")
			{
				...
				if(check != 0)
				{
					var owner = ShipManager.GetOwner(selectedShip.EntityId.ToString());
					if(owner == SenderSteamId)
					{
						var removed = ShipManager.Remove(selectedShip.EntityId.ToString(), SenderSteamId);
						if(removed)
					...
					if(check == Amount)
					{
						...
						// Background processing occurs within this block.
						{
							...
							try
							{
								...
								gridCount = grids.Count;
								var owner = ShipManager.GetOwner(selectedShip.EntityId.ToString());
								var ownerid = ShipManager.GetOwnerID(selectedShip.EntityId.ToString());
								foreach (var grid in grids)
								{
									var blocks = new List<IMySlimBlock>();
									grid.GetBlocks(blocks);
								...
								if(owned > (terminalBlocks / 2))
								{
									...
									if (accountbuyer.BankBalance >= Amount)
									{
										...
										var removed = ShipManager.Remove(selectedShip.EntityId.ToString(), owner);
